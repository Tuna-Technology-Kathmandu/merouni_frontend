...
//   return (
//     <div
//       className={`fixed flex flex-col top-0 left-0 w-screen md:w-full transition-all duration-300 z-50 ${
//         searchTag.trim()
    ...
    return (
      <div className="mb-8 px-4 md:px-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
          {items.slice(0, 3).map((item, index) => (
            <Link href={`/${categoryPath}/${item.slugs}`} key={index}>
              <div className="cursor-pointer p-4 max-w-sm mx-auto">
                <div className="flex justify-center border-2 rounded-3xl items-center overflow-hidden mb-2 p-4">
                  <img
                    src="/images/hult_prize.png"
                    alt={item.name}
        ...
        <div className="mt-8 px-4 md:px-0 text-center">
          ...
          <ul className="mt-2 space-y-2">
            {popularSearches.map((search, index) => (
              <li
                key={index}
                className="py-2 text-sm md:text-base font-medium cursor-pointer hover:bg-gray-100 hover:text-[#30AD8F] rounded-md px-2"
                onClick={() => handleItemClick(search)}
              >
                {search}
              </li>
