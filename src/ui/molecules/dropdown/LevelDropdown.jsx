...
/**
 * LevelDropdown - A searchable dropdown component for selecting education levels
 *
 * @param {Object} props
 * @param {string|number} props.value - Selected level ID
 * @param {Function} props.onChange - Callback when level is selected (receives level ID as string or '')
 * @param {string} props.placeholder - Placeholder text (default: "Select level")
...
const LevelDropdown = ({
    ...
    const loadLevels = async () => {
      try {
        ...
        const response = await authFetch(
          `${DotenvConfig.NEXT_APP_API_BASE_URL}/level?limit=500`,
          { cache: 'no-store' }
        )
        if (response.ok) {
  ...
  const filteredLevels = useMemo(() => {
    ...
    return levels.filter(
      (level) =>
        level.title && level.title.toLowerCase().includes(term)
    )
  }, [levels, searchTerm])
    ...
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      >
        <span className={value ? 'text-gray-900' : 'text-gray-500'}>
          {loading ? 'Loading...' : selectedLevelLabel}
        </span>
      </button>
      <ChevronDown
