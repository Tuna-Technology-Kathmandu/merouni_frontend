...
import { getColleges } from '../colleges/actions'
...
const VacanciesPage = () => {
    ...
    useEffect(() => {
        const getFilterOptions = async () => {
            try {
                const { colleges: collegeList } = await getColleges(1, { limit: 100 })
    ...
    const loadVacancies = useCallback(async (page = 1, search = '', collegeId = '') => {
        setLoading(true)
        try {
            const response = await getVacancies(page, search, collegeId)
    ...
    useEffect(() => {
        loadVacancies(pagination.currentPage, debouncedSearch, selectedCollege)
    }, [debouncedSearch, pagination.currentPage, selectedCollege, loadVacancies])
                    ...
                    <div className='flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12'>
                        <div>
                            ...
                            <p className='text-gray-500 max-w-xl font-medium text-lg mt-2'>
                                Browse open positions from various colleges and institutions. Find the role that best fits your skills.
                            </p>
                        </div>
                    ...
                    <div className='bg-white rounded-[32px] p-8 shadow-[0_2px_15px_rgba(0,0,0,0.02)] border border-gray-100 mb-12'>
                        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6'>
                            ...
                            <div className='lg:col-span-8'>
                                <label className='block text-[10px] uppercase tracking-widest font-bold text-gray-400 mb-2'>Search Vacancies</label>
                            ...
                            <div className='lg:col-span-4'>
                                <label className='block text-[10px] uppercase tracking-widest font-bold text-gray-400 mb-2'>College</label>
                                <div className='relative group'>
                                    ...
                                    >
                                        ...
                                        {colleges.map((college) => (
                                            <option key={college.collegeId} value={college.collegeId}>{college.name}</option>
                                        ))}
                                    </select>
                                </div>
                            </div>
                    ...
                    {!loading && !isScrolling && (
                        <div className='mb-8 px-2'>
                            <p className='text-sm text-gray-500 font-semibold'>
                                Showing <span className='text-gray-900'>{vacancies.length}</span> of <span className='text-gray-900'>{pagination.totalCount}</span> results
                            </p>
                        </div>
                    )}
                    ...
                    {loading || isScrolling ? (
                        ...
                        <div className='bg-white rounded-[32px] border border-gray-100 border-dashed py-20'>
                            <EmptyState
                                ...
                                description={
                                    searchTerm || selectedCollege
                                        ? 'No vacancies match your current filter criteria. Try adjusting your search or filters.'
