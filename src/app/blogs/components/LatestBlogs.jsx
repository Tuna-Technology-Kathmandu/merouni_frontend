...
const LatestBlogs = ({ image, title, date, description }) => {
  return (
    <div className='relative rounded-2xl shadow-lg min-w-full h-full p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden text-white group cursor-pointer'>
      {/* Background Image */}
      <img
        className='absolute inset-0 bg-cover bg-center w-full h-full rounded-2xl transform transition-transform duration-700 object-cover'
        src={image}
        alt={title}
      />
