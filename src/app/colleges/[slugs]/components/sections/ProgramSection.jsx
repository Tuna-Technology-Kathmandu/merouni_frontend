      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6'>
        {college?.collegeCourses.length > 0 ? (
          college.collegeCourses.map((course, index) => (
            <div
              key={index}
              className='group bg-white border border-gray-100 rounded-2xl p-6 shadow-[0_2px_15px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.05)] transition-all duration-300 flex flex-col justify-between'
            >
              <div>
                <h3
              ...
              <button
                ...
                type='button'
                className='w-full sm:w-max bg-[#0A6FA7] hover:bg-[#085e8a] text-white text-sm font-medium py-2.5 px-6 rounded-xl transition-all active:scale-95 shadow-md'
              >
                Apply Now
              </button>
            </div>
