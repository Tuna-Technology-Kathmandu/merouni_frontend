      ...
      <div className='flex flex-row overflow-x-auto pb-4 -mx-2 px-2 sm:mx-0 sm:px-0 no-scrollbar sm:grid sm:grid-cols-2 gap-4 sm:gap-8'>
        {college.facilities.map((item, index) => {
          return (
            <div
              key={index}
              className='group flex flex-col p-4 sm:p-6 rounded-md sm:rounded-2xl bg-gray-50 border border-gray-100 hover:shadow-md transition-all duration-300 min-w-[200px] sm:min-w-0'
            >
              <div className='flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4'>
                {item?.icon && (
                  <div className='w-10 h-10 sm:w-12 sm:h-12 rounded-md sm:rounded-md overflow-hidden bg-white flex-shrink-0 p-1.5 sm:p-2 border border-gray-100 group-hover:bg-[#30AD8F]/5 transition-colors'>
                    <img
                      src={item.icon}
                      alt={item?.title}
